...
# #     network_mode: host
# #     command: python /aps-mini-apps/build/python/streamer-daq/DAQStream.py --mode 1 --simulation_file /aps-mini-apps/data/tomo_00058_all_subsampled1p_s1079s1081.h5 --d_iteration 100 --publisher_addr tcp://*:{data_port} --iteration_sleep=0 --projection_sleep=0 --synch_addr tcp://*:{sync_port} --synch_count 1"""
# #
...
#     #extra = f"globus-streams-launch -p {port} {shlex.quote(tunnel_id)} " if service == "daq" else ""
#     #cp = run_subprocess(
#     cp = popen_subprocess(
#         host, None,
...
def _start_service(
    ...
    for i in range(parallel):
        #log_file = shlex.quote(f"{out_dir}/{app}-{service}.log")
        wrapper_cmd = f"globus-streams-launch -p {port} " if service == "daq" else "globus-streams-launch "
        cp = popen_subprocess(
            host, None,
            f"cd {shlex.quote(module_path)} && " 
            f"{wrapper_cmd} {$tunnel_id} docker run -d "
            f"--rm --network host --name daq-{i}   seenv/aps-mini-apps-daq:latest "   
    ...
    for host in cfg.hosts.ep.values():
        ...
        if host == cfg.hosts.ep["listener"]:
            ...
            if status != parallel and retry < retries:
                ...
                logging.debug("MINI: Started DAQ on the %s", host.upper())
                break
        if host == cfg.hosts.ep["initiator"]:
            _start_service()
