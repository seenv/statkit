...
def run_iperf_baseline(cfg: Config, *, idx: int, total_runs: int, timeout: int, 
    ...
    try:
        ...
        base_start_iperf_server(cfg, listener_host, port, temp_file, "iperf_base", numa, output_dir)
        time.sleep(cfg.sleep)           # it takes more for them to initiates! TODO: find a better way
        # run iperf client
        ...
        base_start_iperf_client(cfg, initiator_host, cfg.listener_pub, port, parallel, arg, temp_file, "iperf_base", numa, output_dir, timeout) 
        if not cfg.is_test:   
...
def run_rsync(cfg: Config, *, idx: int, total_runs: int, timeout: int, temp_file: str, 
    ...
    try:
        ...
        if encrypt == 1:
            ...
            start_rsync_daemon(
                cfg, initiator_host, numa, output_dir, cfg.rsync_port, timeout,
