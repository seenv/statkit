...
def stop_rsync_daemon(
    cfg: Config, host: str, parallel: int, module_path: str = "/tmp/temp_files", check: bool = True
) -> None:
...
def start_rsync_daemon_gst(
    cfg: Config, src_host: str, start_port: int, stream_ids: Sequence[str], 
    parallel:int, numa: str, out_dir: str, timeout: int, app: str, 
...
def start_rsync_transfer_gst(
    ...
    numa: str, arg: int, files: list[str], out_dir: str, timeout: int, app: str, 
    module_name: str = "transfer", module_path: str = "/tmp/temp_files", check: bool = True,
...
# -------------------------------------------------------------------------------
# Rsync SciStream
def start_rsync_daemon_sci(
    cfg: Config, src_host: str, listen_ep_ports: Sequence[int], stream_ids: Sequence[str], 
    parallel:int, numa: str, out_dir: str, timeout: int, app: str, module_name: str = "transfer", 
    module_path: str = "/tmp/temp_files", check: bool = True,
) -> list[subprocess.Popen[str]]:
    ...
    for i, (listen_port, stream_id) in enumerate(zip(listen_ep_ports, stream_ids)):
        cp = popen_subprocess(
            ...
            f"rsync --daemon -vvv --config={shlex.quote(module_path)}/rsyncd-{i}.conf --port={listen_port} "
            f"--log-file={shlex.quote(out_dir)}/{shlex.quote(app)}-daemon-{i}.log; ",
            localhost=cfg.localhost,
            #timeout=timeout,
        )
        processes.append(cp)
        logging.debug("RSCI: Started rsync daemon on %s:%s", src_host.upper(), listen_port)
    return processes


def start_rsync_transfer_sci(
    cfg: Config, src_host: str, dst_host: str, listen_ip: str, 
    stream_ids: Sequence[str], listen_ports: Sequence[int], parallel: int, 
    numa: str, arg: int, files: list[str], out_dir: str, timeout: int, app: str, 
    module_name: str = "transfer", module_path: str = "/tmp/temp_files", check: bool = True,
    ...
    for i, cp in enumerate(processes):
        stdout, stderr = cp.communicate()
        if cp.returncode != 0:
            logging.error("RSCI: Process failed: %s", stderr)

        logging.debug("RSCI: Completed rsync daemon transfer of file %s %s", files[i], stdout)
